...
Usage:
    python alpaca_contribute.py              # live execution with diagnostics
    python alpaca_contribute.py --dry-run    # simulate orders without placing them
    python alpaca_contribute.py --amount 120 # override contribution amount in USD
...
# Exits the script if the market is closed today
def check_market_open(client):
    clock = client.get_clock()
    if not clock.is_open:
        print("Market is closed today. Use --dry-run to simulate without placing orders. Exiting.")
        sys.exit(0)

# Returns a dictionary of {ticker: market_value} for all current positions
def get_position_values(client) -> dict[str, float]:
...
def layer2(equity_amount: float, prices: pd.DataFrame, bm_mean: float, bm_sd: float) -> tuple[dict[str, float], dict[str, float]]:
    # Score each sub-bucket using only ETFs in ETF_SLOT_RATIOS
    raw_scores = {}
    for sub in SUB_BUCKETS:
        ...
        raw_scores[sub] = float(np.mean(scores))
...
def place_orders(client, etf_allocations: dict[str, float], dry_run: bool = True):
...
def main():
    # ── ARGUMENTS ──
    parser = argparse.ArgumentParser(description="Alpaca weekly contribution script")
    ...
    # ── MARKET DATA ──
    position_values = get_position_values(client)
    ...
    # ── EXECUTE ──
    place_orders(client, l3, dry_run=args.dry_run)
